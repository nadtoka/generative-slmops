...
3. **Host-Level Trust Configuration**: Target Linux hosts are configureed just once to trust the public root signing key of Vault's CA via the `TrustedUserCAKeys` directive inside `/etc/ssh/sshd_config`. 
